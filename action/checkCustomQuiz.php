...
if (isset($_POST['quizId'])) {
    $quizId = $_POST['quizId'];
    if (isset($_SESSION['custom_quiz' . $quizId])) {
        $time = time() - ($_SESSION['custom_quiz' . $quizId]);
        unset($_SESSION['quiz' . $quizId]);
        $data = NULL;
        $query = "SELECT q.`questionId`,q.correctoption FROM content_question q , user_customquizquestion dq 
            ...
            if (isset($_POST['questionId' . $i]) && is_numeric($_POST['questionId' . $i])) {
                ...
                if (isset($_POST['quizQuesRadio' . $i]) && is_numeric($_POST['quizQuesRadio' . $i])) {
                    ...
                    if ($sql == "") {
                        ...
                        VALUES ('$userId','$questionId','$answer','$isCorrect','$timeTaken','$quizId','4')";
                    } else {
                        $sql = $sql . ",('$userId','$questionId','$answer','$isCorrect','$timeTaken','$quizId','4')";
                    }
                }
            } else {
                break;
        ...
        $sql2 = "UPDATE user_customquiz SET total='$total', attempted='$attempted', correct='$correct', `timeTaken`='$time', status = 1
                WHERE userid = '$userId' AND id = '$quizId'";
